...
public class CartController {
	...
	public String addProduct(@ModelAttribute soldProduct soldProduct,@PathVariable("id") Integer id,@AuthenticationPrincipal UsersDetails userD,RedirectAttributes redirAttrs) {
		...
		prodServ.addSoldProduct(soldProduct,user,id);
	...
	public String checkout(@AuthenticationPrincipal UsersDetails userD,@ModelAttribute proccesedShoppingCart proCart,@ModelAttribute soldProduct soldProduct ) {
		User user = userRepo.findByEmail(userD.getUsername());
		ShoppingCart cart = shopRepo.findByUser(user);
